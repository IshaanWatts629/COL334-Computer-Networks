...
/*
    ...
    1. Below is a simple dumbbell topology, two client Node0 and Node1 on
    the left side of the dumbbell and server nodes Node2 and Node3 on the
    right side of the dumbbell. Node4 and Node5 forms the bridge of the
    dumbbell. 

    2. TCP socket instance on Node0 that will connect to Node2.

    3. UDP socket instance on Node1 that will connect to Node3.

    4. TCP application starts at time 1s.

    5. UDP application starts at time 20s at rate 250Kbps such that it clogs
    half the dumbbell bridge's link capacity.

    6. UDP application's rate changes at time 30s to rate 500Kbps
...
{
  std::string lat = "2ms";
  std::string rate = "600kb/s"; // P2P link
...
//
  ...
  Ptr<Socket> ns3TcpSocket = Socket::CreateSocket (c.Get (1), TcpSocketFactory::GetTypeId ()); //source at n0
 ...
 // ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeCallback (&CwndChange));
  ...
  app->Setup (ns3TcpSocket, sinkAddress, 1040, 100000, DataRate ("300Kbps"));
  ...
  Ptr<Socket> ns3UdpSocket = Socket::CreateSocket (c.Get (0), UdpSocketFactory::GetTypeId ()); //source at n1

  // Create UDP application at n1
  Ptr<MyApp> app2 = CreateObject<MyApp> ();
  app2->Setup (ns3UdpSocket, sinkAddress2, 1040, 100000, DataRate ("300Kbps"));
  c.Get (0)->AddApplication (app2);
  app2->SetStartTime (Seconds (30.));
  app2->SetStopTime (Seconds (100.));

// Increase UDP Rate
  Simulator::Schedule (Seconds(60.0), &IncRate, app2, DataRate("600kbps"));

  // Flow Monitor
   AsciiTraceHelper asciiTraceHelper;
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("evaluation.txt");
